...
import "./ui/ScanScreen.css"; // макет + фікс білої смуги
...
export default function ScanScreen() {
  ...
  // Мова інтерфейсу (localStorage)
  const [locale, setLocale] = useState<"uk" | "en">(() => {
    try { return (localStorage.getItem("sghelper:locale") === "en") ? "en" : "uk"; }
    catch { return "uk"; }
  });
  ...
  async function run<T>(fn: () => Promise<T>) {
    ...
    setError(undefined);
    try { return await fn(); }
    catch (e: any) { setError(e?.toString?.() ?? "Помилка"); return undefined as any; }
    finally { setBusy(false); }
  }

  async function refreshAccounts() {
    try {
      const hints = await invoke<AccountHint[]>("detect_accounts");
      setAccounts(hints || []);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const cached = await invoke<LibraryGame[]>("load_library_cache").catch(() => []);
      ...
      // онбординг
      let done = false;
      try { done = localStorage.getItem("sghelper:onboarding_done") === "1"; } catch {}
  ...
  // Збереження мови
  useEffect(() => {
    try { localStorage.setItem("sghelper:locale", locale); } catch {}
  }, [locale]);
  ...
  async function openHowToKey() {
    await run(async () => { await invoke("open_apikey_page"); });
  }
  async function installBrowserHelper() {
    const dir = await invoke<string>("ensure_browser_helper").catch(() => "");
    await invoke("open_extensions_manager").catch(() => {});
    if (dir) {
      alert(`Файли помічника збережено:\n${dir}\n\nУ браузері увімкни «Режим розробника» та додай цю папку як розширення.`);
    }
  }
  async function pasteFamilyFromClipboard() {
    try { const txt = await navigator.clipboard.readText(); setFamilyIds(txt.trim()); } catch {}
  }
  async function clearCache() {
    setGames([]); await invoke("save_library_cache", { games: [] }).catch(() => {});
  }
  async function scanUnified() {
    if (!apiKey.trim()) return setError(locale === "en" ? "Specify Steam API key in Settings." : "Вкажіть Steam API key у Налаштуваннях.");
    if (!steamId.trim()) return setError(locale === "en" ? "Select or enter SteamID64 in Settings." : "Оберіть або введіть SteamID64 у Налаштуваннях.");
    const fam = familyIds.split(/[, \n\r\t]+/g).map((s) => s.trim()).filter(Boolean);
    const data = await run(async () => {
      return await invoke<LibraryGame[]>("scan_library_unified", {
        api_key: apiKey,
    ...
    if (data) {
      setGames(data);
      invoke("save_settings", { s: { api_key: apiKey.trim(), main_steam_id64: steamId.trim(), family_ids: fam } }).catch(() => {});
      invoke("save_library_cache", { games: data }).catch(() => {});
  ...
  const filtered = useMemo(() => {
    ...
    return q
      ? games.filter((g) => (g.name || "").toLowerCase().includes(q) || String(g.appid).includes(q))
      : games;
  }, [games, search]);
  ...
  const sorted = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "playtime": arr.sort((a, b) => (b.playtime_minutes ?? 0) - (a.playtime_minutes ?? 0)); break;
      case "installed": arr.sort((a, b) => Number(b.installed) - Number(a.installed)); break;
    }
    return arr;
  }, [filtered, sortBy]);

  const totalInstalled = useMemo(() => games.filter((g) => g.installed).length, [games]);
      ...
      <div className="sg-container">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
          ...
          <div style={{ marginLeft: "auto" }}>
            <button
              onClick={() => { setShowSettings(true); setSettingsTab("params"); }}
              style={{
                padding: "10px 16px", borderRadius: 10, background: steam.accent, color: "#0b1016",
                border: "none", cursor: "pointer", fontWeight: 700, boxShadow: "0 6px 18px rgba(102,192,244,.25)",
              }}
            >
        ...
        <div
          style={{
            ...
            border: `1px solid ${steam.border}`, boxShadow: "0 26px 80px rgba(0,0,0,.38)",
          }}
        >
          <div
            style={{
              display: "flex", alignItems: "center", gap: 10, padding: "8px 10px 12px",
          ...
          >
            ...
            <div
              style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center", color: steam.textMuted, fontSize: 12 }}
              title={locale === "en" ? "Library status (for prompts context)" : "Статус бібліотеки (для контексту підказок)"}
            >
              <span style={{ background: steam.chipBg, borderRadius: 999, padding: "4px 8px" }}>
                {locale === "en" ? "Cached" : "У кеші"}: <b style={{ color: steam.text }}>{games.length}</b>
              </span>
              <span style={{ background: steam.chipBg, borderRadius: 999, padding: "4px 8px" }}>
                {locale === "en" ? "Installed" : "Встановлено"}: <b style={{ color: steam.text }}>{totalInstalled}</b>
              </span>
            </div>
          </div>
          ...
          <div
            ...
            style={{
              marginTop: 10, background: steam.panelSoft, border: `1px solid ${steam.border}`,
          ...
          <div style={{ fontSize: 12, color: steam.textMuted, marginTop: 10, paddingLeft: 2 }}>
            {locale === "en"
            ...
            {error ? <span style={{ color: "#ffd2d2", marginLeft: 8 }}>• {error}</span> : null}
          </div>
        </div>
      </div>
      ...
      {showSettings && (
        ...
        >
          <div
            ...
            style={{
              width: "min(1020px, 96vw)", maxHeight: "88vh", overflow: "hidden",
              borderRadius: 14, background: steam.panel, border: `1px solid ${steam.border}`,
              boxShadow: "0 18px 48px rgba(0,0,0,.45)", display: "flex", flexDirection: "column",
            }}
          >
            {/* Header */}
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", borderBottom: `1px solid ${steam.border}` }}>
              <div style={{ fontWeight: 700, color: steam.text }}>{locale === "en" ? "Settings" : "Налаштування"}</div>
              <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
                <button
                  onClick={() => setSettingsTab("params")}
                  style={{
                    padding: "8px 10px", borderRadius: 8,
                    border: `1px solid ${settingsTab === "params" ? steam.accent : steam.border}`,
                    background: settingsTab === "params" ? "#11273a" : steam.panelSoft,
                    color: steam.text, cursor: "pointer",
                  }}
                >
                ...
                <button
                  ...
                  style={{
                    padding: "8px 10px", borderRadius: 8,
                    border: `1px solid ${settingsTab === "scan" ? steam.accent : steam.border}`,
                ...
                <button
                  ...
                  style={{
                    padding: "8px 10px", borderRadius: 8,
                    border: `1px solid ${settingsTab === "library" ? steam.accent : steam.border}`,
                ...
                <button
                  onClick={() => setShowSettings(false)}
                  style={{
                    padding: "8px 10px", borderRadius: 8, border: `1px solid ${steam.border}`,
                    background: steam.panelSoft, color: steam.text, cursor: "pointer",
                  }}
                >
            ...
            <div style={{ padding: 16, overflow: "auto" }}>
              {settingsTab === "params" && (
                <div style={{ display: "grid", rowGap: 12 }}>
                  {/* Локальний ШІ (інфо-картка) */}
                  <div
                    style={{
                      border: `1px solid ${steam.border}`, background: steam.panelSoft,
                      borderRadius: 10, padding: 12, color: steam.text,
                    }}
                  >
                    <div style={{ fontWeight: 700, marginBottom: 6 }}>
                      {locale === "en" ? "AI" : "ШІ"}
                    </div>
                    <div style={{ fontSize: 13, color: steam.textMuted, lineHeight: 1.5 }}>
                      {locale === "en"
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 10, alignItems: "center" }}>
                    ...
                    <select
                      ...
                      style={{
                        ...
                        border: `1px solid ${steam.inputBorder}`, background: steam.inputBg, color: steam.text,
                      }}
                    >
                  ...
                  {/* Family */}
                  <div style={{ display: "grid", gridTemplateColumns: "200px 1fr auto auto auto", gap: 10, alignItems: "center" }}>
                    ...
                    <button
                      ...
                      title={locale === "en" ? "Helper extension to copy SteamIDs" : "Помічник для копіювання SteamID"}
                      style={{
                        padding: "10px 12px",
                        borderRadius: 10,
                        background: steam.panelSoft,
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 4, minHeight: 18 }}>
                    <div style={{ fontSize: 12, color: steam.textMuted }}>
                      {savedHint === "saving"
                        ? (locale === "en" ? "Saving…" : "Збереження…")
                        : savedHint === "saved"
                        ? (locale === "en" ? "Saved ✓" : "Збережено ✓")
                        : ""}
                    </div>
                    <div style={{ marginLeft: "auto" }}>
                      <button
              ...
              {settingsTab === "scan" && (
                <div style={{ display: "grid", rowGap: 12 }}>
                  ...
                  <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                    ...
                    >
                      {busy ? (locale === "en" ? "Scanning…" : "Сканую…")
                           : (locale === "en" ? "Scan library" : "Сканувати бібліотеку")}
                    </button>
                    <button
                      onClick={clearCache}
                      disabled={busy}
                    ...
                    <div style={{ marginLeft: "auto", display: "flex", gap: 12 }}>
                      <div style={{ background: steam.chipBg, borderRadius: 999, padding: "6px 10px", fontSize: 13 }}>
                        {locale === "en" ? "Cached" : "У кеші"}: <b>{games.length}</b>
                      </div>
                      <div style={{ background: steam.chipBg, borderRadius: 999, padding: "6px 10px", fontSize: 13 }}>
                        {locale === "en" ? "Installed" : "Встановлено"}: <b>{games.filter((g) => g.installed).length}</b>
                      </div>
                    </div>
                  </div>
                  {error && (
...
function LibraryView({
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gap: 8, gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))" }}>
        {games.map((g) => (
          ...
          >
            ...
            <div style={{ opacity: 0.7, fontSize: 12, marginTop: 4, color: "#8aa2b5" }}>
              appid: {g.appid}
            </div>
            <div style={{ opacity: 0.9, fontSize: 12, marginTop: 6 }}>
              {t("time")}: {g.playtime_minutes != null ? `${Math.round((g.playtime_minutes ?? 0) / 60)} ${t("hoursShort")}` : "—"}
            </div>
            {g.shared_from && (
      ...
      {games.length === 0 && (
        <div style={{ opacity: 0.7, marginTop: 12, color: "#8aa2b5" }}>
          {t("noResults")}
        </div>
      )}
    </div>
